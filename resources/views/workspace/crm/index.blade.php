<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('status'))
                <div class="bg-emerald-600 border border-emerald-700 text-white font-black text-xs uppercase tracking-tight rounded-2xl p-4 mb-6 shadow-md flex items-center gap-2 mx-4 sm:mx-0">
                    <span>👍</span>
                    <p>{{ session('status') }}</p>
                </div>
            @endif

            <form action="{{ route('workspace.crm.index') }}" method="GET" class="mb-10 px-4 sm:px-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 px-4 sm:px-0">
                @forelse($clients as $client)
                    <div class="bg-white rounded-3xl shadow-lg border-2 border-slate-300 overflow-hidden flex flex-col hover:border-slate-900 hover:shadow-2xl transition-all group">
                        ...
                        <div class="grid grid-cols-2 border-t-4 border-slate-200 bg-white">
                            @if($client->phone)
                                <a href="tel:{{ $client->phone }}" class="flex flex-col items-center justify-center py-6 border-r-2 border-slate-200 hover:bg-emerald-50 transition action-group text-decoration-none">
                            ...
                            @if($client->address)
                                <button onclick="shareDirections('{{ addslashes($client->name) }}', '{{ addslashes($client->address) }}')" class="flex flex-col items-center justify-center py-6 border-slate-200 hover:bg-indigo-50 transition action-group bg-transparent border-0 cursor-pointer outline-none">
    ...
    <script>
        function shareDirections(name, address) {
            ...
            const text = `Job Site Info for ${name}:\n\nAddress: ${address}\n\nGoogle Maps Directions: https://www.google.com/maps/search/?api=1&query=${encodedAddress}`;
