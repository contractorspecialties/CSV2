    ...
    <div class="max-w-4xl mx-auto px-4 py-10 space-y-8">
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6">
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                ...
                <div class="space-y-2">
                    <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                        <span class="bg-orange-50 text-[#f58613] text-[9px] font-black tracking-widest uppercase px-2.5 py-1 rounded-md border border-orange-100 shadow-sm">✓ Verified Pro</span>
                    ...
                    @if(!empty($company->city))
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">
                            📍 Location: <span class="text-slate-900 font-black">{{ $company->city }}{{ !empty($company->state) ? ', ' . strtoupper($company->state) : '' }}</span>
                            @if(!empty($company->service_radius_miles))
                                <span class="text-slate-400 font-normal font-sans">({{ $company->service_radius_miles }} Mile Service Radius)</span>
                            @endif
                        </p>
                    @endif
                </div>
            ...
            <div class="bg-slate-50 border border-slate-200/60 rounded-2xl p-4 text-center shrink-0 w-full sm:w-auto">
                ...
                <div class="text-[9px] font-black uppercase text-slate-400 tracking-wider">Customer Rating</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                @if(!empty($company->computed_bio_long))
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-3">
                        <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider">Our Company History</h3>
                ...
                @if(is_array($serviceTags) && count($serviceTags) > 0)
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
                        <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider">What We Do Best</h3>
                ...
                @if(!empty($galleryImages) && count($galleryImages) > 0)
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
                        <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider">Recent Job Photos</h3>
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($galleryImages as $index => $img)
                                <div @click="lightboxOpen = true; activeIndex = {{ $index }}"
                                    ...
                                    <img src="{{ asset($img) }}" class="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-300" alt="Showcase Photo">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
                    <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider border-b border-slate-100 pb-2">Business Details</h3>
                    ...
                    <div class="space-y-3.5 text-xs font-medium">
                        @if(!empty($company->license_number))
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <div class="font-black text-slate-950 uppercase text-[10px]">License Number</div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <div class="font-black text-slate-950 uppercase text-[10px]">Years in Business</div>
                                <div class="text-slate-500 font-bold mt-0.5">{{ $company->computed_experience }} Years Active</div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3">
                            <span class="text-base select-none">💬</span>
                            <div>
                                <div class="font-black text-slate-950 uppercase text-[10px]">Response Time</div>
                                <div class="text-slate-500 font-bold mt-0.5">Usually responds {{ $company->typical_response_time ?? 'within 24 hours' }}</div>
                            </div>
                        </div>
                        ...
                        @if(!empty($company->warranty_details))
                            <div class="flex items-start gap-3 border-t border-slate-100 pt-3.5">
                                <span class="text-base select-none">🛡️</span>
                                <div>
                                    <div class="font-black text-slate-950 uppercase text-[10px]">Warranty Policy</div>
                ...
                @if(!empty($company->deposit_rules))
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-2">
                        <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider border-b border-slate-100 pb-2">Deposit & Payment Rules</h3>
                ...
                @if($hasSocials)
                    <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-3">
                        <h3 class="text-xs font-black uppercase text-slate-400 tracking-wider border-b border-slate-100 pb-2">Verified Reviews</h3>
                        <div class="flex flex-col gap-2">
                            ...
                            @if(!empty($socialLinks['facebook']))
                                <a href="{{ $socialLinks['facebook'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 p-2 rounded-xl bg-slate-50 border border-slate-200 text-[11px] font-bold text-slate-700 hover:bg-slate-100 transition-colors text-decoration-none">
                                    <span>🔵</span> Follow Our Facebook Page
                                </a>
                            @endif
                            @if(!empty($socialLinks['yelp']))
                                <a href="{{ $socialLinks['yelp'] }}" target="_blank" rel="noopener" class="flex items-center gap-2 p-2 rounded-xl bg-slate-50 border border-slate-200 text-[11px] font-bold text-slate-700 hover:bg-slate-100 transition-colors text-decoration-none">
                                    <span>🔴</span> Check Our Yelp Reviews
                                </a>
                            @endif
                        </div>
                    </div>
                ...
                @if(!empty($routingLine))
                    <div class="bg-slate-950 border border-slate-900 rounded-3xl p-6 shadow-xl text-white text-center space-y-4">
                        <div class="space-y-1">
                            <h4 class="font-black text-xs text-[#f58613] uppercase tracking-widest">Ready to Start?</h4>
                            <p class="text-[11px] text-slate-400 font-medium max-w-[200px] mx-auto leading-normal">Call us directly to schedule an estimate or request service layout runs.</p>
                        </div>
    ...
    <div x-show="lightboxOpen"
        ...
        <div class="relative z-20 w-full max-w-5xl bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl grid grid-cols-1 md:grid-cols-3 max-h-[90vh] md:max-h-[80vh]">
            ...
            <div class="md:col-span-2 bg-slate-950 relative flex items-center justify-center min-h-[300px] md:min-h-[450px] overflow-hidden">
                <img :src="imagePool[activeIndex]" class="max-w-full max-h-[50vh] md:max-h-[75vh] object-contain select-none" alt="Showcase Image">
            ...
            <div class="bg-white p-6 md:p-8 flex flex-col justify-between overflow-y-auto border-t md:border-t-0 md:border-l border-slate-200">
                <div class="space-y-5">
                    <div>
                        ...
                        <h4 class="text-base font-black uppercase text-slate-950 tracking-tight mt-1.5">Project Photos</h4>

                        @if(!empty($company->city))
                            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                                Location: <span class="text-slate-900 font-black">{{ $company->city }}, {{ strtoupper($company->state ?? '') }}</span>
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-3.5 text-xs font-semibold text-slate-600 shadow-inner">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <div class="text-[9px] font-black uppercase text-slate-400 tracking-wider">Company</div>
                        ...
                        @if(!empty($company->license_number))
                            <div class="flex items-center gap-2.5 border-t border-slate-200/50 pt-2.5">
                                ...
                                <div>
                                    <div class="text-[9px] font-black uppercase text-slate-400 tracking-wider">License</div>
                        ...
                        @if(!empty($company->warranty_details))
                            <div class="flex items-center gap-2.5 border-t border-slate-200/50 pt-2.5">
                                <span class="text-sm select-none">✨</span>
                                <div>
                                    <div class="text-[9px] font-black uppercase text-slate-400 tracking-wider">Warranty</div>
                    ...
                    <p class="text-[11px] text-slate-400 font-medium leading-relaxed italic bg-slate-50/50 p-3 rounded-xl border border-slate-200/40">
                        💡 This photo shows real project quality and field work handled locally by our team.
                    </p>
                </div>
